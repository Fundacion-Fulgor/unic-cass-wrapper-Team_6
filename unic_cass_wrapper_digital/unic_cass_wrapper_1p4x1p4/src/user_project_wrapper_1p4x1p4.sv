...
module user_project_wrapper_1p4x1p4 (
    inout wire io_clock_PAD,
    inout wire io_reset_PAD,
    inout wire [8:0] ui_PAD,
    inout wire [8:0] uo_PAD
);
    wire io_clock_p2c;
    wire io_reset_p2c;
    wire [8:0] ui_PAD2CORE;
    wire [8:0] uo_CORE2PAD;
...
//
    ...
    generate
    for (genvar i=0; i<9; i++) begin : sg13g2_IOPadIn_ui
        sg13g2_IOPadIn ui (
            `ifdef USE_POWER_PINS
            .vss    (VSS),
            .vdd    (VDD),
    ...
    generate
    for (genvar i=0; i<9; i++) begin : sg13g2_IOPadOut30mA_uo
